...
$(document).on('click', "#assetpurchasepayment", function (e) {
    
    e.preventDefault();

    var pyurl = baseurl + 'transactions/asset_purchase_payment';
...
$(document).on('click', ".delete-object-vartions", function (e) {
    e.preventDefault();
    $('#object-id').val($(this).attr('data-object-id'));

    $(this).closest('tr').attr('id', $(this).attr('data-object-id'));
    $('#varition_delete_model').modal({backdrop: 'static', keyboard: false});

});

$("#delete-confirm-varition").on("click", function () {
    var o_data = 'deleteid=' + $('#object-id').val();
    var action_url = $('#varition_delete_model #action-url-dlt').val();
    $('#' + $('#object-id').val()).remove();
    removeObject(o_data, action_url);
});


$(document).on('click', ".deactivate-object", function (e) {
    e.preventDefault();
    $('#object-id').val($(this).attr('data-object-id'));

    $(this).closest('tr').attr('id', $(this).attr('data-object-id'));
    $('#deactivate_model').modal({backdrop: 'static', keyboard: false});
...
$(document).on('click', ".activate-object", function (e) {
    ...
    $(this).closest('tr').attr('id', $(this).attr('data-object-id'));
    $('#activate_model').modal({backdrop: 'static', keyboard: false});
...
$("#deactivate-confirm").on("click", function () {
	var o_data = 'deactivateid=' + $('#object-id').val();
    var action_url = $('#deactivate_model #action-url').val();
    //$('#' + $('#object-id').val()).remove();
    changeStaus(o_data, action_url);
});

$("#activate-confirm").on("click", function () {
	var o_data = 'activateid=' + $('#object-id').val();
    var action_url = $('#activate_model #action-url').val();
    //$('#' + $('#object-id').val()).remove();
    changeStaus(o_data, action_url);
});
...
function changeStaus(action, action_url) {
    if ($("#notify").length == 0) {
...
$("#submita-data").on("click", function (e) {
    e.preventDefault();
    $(this).hide();
    ...
    var action_url = $('#action-url').val();
    addObject(o_data, action_url);
    setTimeout(function(){  $("#submita-data").show(); }, 1000);
