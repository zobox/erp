...
class Quote_model extends CI_Model
{
	...
	public function insertQcExcel(){
		$data = array();
		$data['status'] = 1;
		$data['date_created'] = date('Y-m-d h:i:s');
		$data['logged_user_id'] = $_SESSION['id'];		
		$this->db->insert('tbl_qc_data_excel',$data);
		return $this->db->insert_id();
	}
	
	public function import_qc($data){		
		foreach($data as $key=>$row){
			$this->db->insert('tbl_qc_data',$row);	
			//echo $this->db->last_query(); 
		}
		return true;
	}

	public function get_qc_Dropdown(){
		$this->db->select('*');
		$this->db->where('status',1);
        $this->db->from('tbl_qc_data');        
        $query = $this->db->get();
        return $query->result();
	}
	
	
	public function get_qc_data($excel_sheet_id)
    {
		$this->db->where('excel_sheet_id', $excel_sheet_id);
		$this->db->where('status', 1);
	...
	public function get_qc_data_excel()
    {
		$this->db->select('a.*,b.name');
        $this->db->from('tbl_qc_data_excel as a');        
        $this->db->join('geopos_employees as b', 'b.id = a.logged_user_id', 'left');
        $query = $this->db->get();
		$this->db->where('a.status', 1);       
        return $query->result();
    }
	
	public function count_all_qc_data_excel(){
		$this->db->select('a.*,b.name');
        $this->db->from('tbl_qc_data_excel as a');        
        $this->db->join('geopos_employees as b', 'b.id = a.logged_user_id', 'left');
        $query = $this->db->get();
