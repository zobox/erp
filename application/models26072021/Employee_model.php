...
class Employee_model extends CI_Model
{
    ...
    public function update_employee($id, $name, $phone, $phonealt, $address, $city, $region, $country, $postbox, $location, $salary = 0, $department = -1, $commission = 0, $roleid = false,$franchise_id=0)
    {
    ...
    public function add_employee($id, $username, $name, $roleid, $franchise_id=0, $phone, $address, $city, $region, $country, $postbox, $location, $salary = 0, $commission = 0, $department = 0)
    {
		$franchise_id = 0;
        $data = array(
            'id' => $id,
	...
	public function getFranchiseByEmpID($eid)
    {
		$this->db->select('b.*,a.name,a.id as employee_id,c.module,b.franchise_id');		
		$this->db->from("geopos_employees as a");
		$this->db->join('geopos_customers as b', 'b.id = a.franchise_id', 'INNER');			
		$this->db->join('geopos_franchise as c', 'c.id = b.franchise_id', 'INNER');			
		$this->db->where('a.id',$eid);
		$query = $this->db->get();	
