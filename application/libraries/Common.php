...
class Common
{
    ...
    function polist()
    {
        ...
        $query = $this->PI->db->get('univarsal_api');
        $row1 = $query->row_array();
        //$polist = '<option value="' . $row1['key1'] . '">--' . $row1['other'] . '--</option> ';
        $polist .= '<option value="">Select PO Type</option><option value="0">' . $this->PI->lang->line('Asset') . ' ' . $this->PI->lang->line('POO') . '</option>
                                                <option value="1">' . $this->PI->lang->line('JOB WORK') . ' ' . $this->PI->lang->line('POO') . '</option> ';
