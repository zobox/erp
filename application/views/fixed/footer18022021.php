...
<script>
	...
	$('.ecommerce').keyup(function(){
		if($('#inlineRadio2').is(':checked')) { 
			...
			if($('#flipkart_price').val() != null && $('#flipkart_price').val() != '' && $('#flipkart_price').val() != ' '){
				var fp = parseFloat($('#flipkart_price').val()); 
				i++;
			}else{
				var fp = 0;
			}
			if($('#amazon_price').val() != null && $('#amazon_price').val() != '' && $('#amazon_price').val() != ' '){
				var ap = parseFloat($('#amazon_price').val()); 
				i++;
			}else{
				var ap = 0;
			}
			if($('#snapdeal_price').val() != null && $('#snapdeal_price').val() != '' && $('#snapdeal_price').val() != ' '){
				var sdp = parseFloat($('#snapdeal_price').val()); 
				i++;
			}else{
				var sdp = 0;
			}
			if($('#2gud_price').val() != null && $('#2gud_price').val() != '' && $('#2gud_price').val() != ' '){
				var twogp = parseFloat($('#2gud_price').val()); 
				i++;
			}else{
				var twogp = 0;
			}
			if($('#infibeam_price').val() != null && $('#infibeam_price').val() != '' && $('#infibeam_price').val() != ' '){
				var inp = parseFloat($('#infibeam_price').val()); 
				i++;
			}else{
				var inp = 0;
			...
			$('#average_price_p_calc').val((fp+ap+sdp+twogp+inp)/i);
		 }
		 else{
		 	$('#average_price_p_calc').val(0);
		 }
...
/* $('#product_name_d_api').change(function(){
	var term = $(this).val();
	$.ajax({  
	...
	$.ajax({
		...
		success : function(result){	
			...
			if(result!=null){
				...
				//$( "#average_price_d_api" ).val(obj.product_lowest_price);
...
$('#dpcalc').click(function(){
	...
	/* if(discount_type==2){
		...
		var discount = ((average_price_d_api*discount_d)/100);
	}	
	var zo_selling_price = (average_price_d_api-discount);	
