            ...
            <div class="card-body">
                ...
                <div class="row">
                    ...
                    <div class="col-md-8">
                        ...
                        <hr>
                        <h5 class="mb-2"><?= $this->lang->line('Link Account'); ?></h5>
                        <hr>
						
						
                        <form method="post" id="data_form" class="form-horizontal" action="<?php echo base_url() ?>customers/link_account">

                           <input type="hidden" name="custid" value="<?= $custid ?>">
							<?php 										
									$cash_account_id = $details['cash_account_id'];
									$other_account_id = $details['other_account_id'];
							?>

                            <div class="form-group row">
                                <label class="col-sm-3 col-form-label"
                                       for="pay_cat"><?php echo $this->lang->line('Cash') ?></label>

                                <div class="col-sm-9">
                                    <select name="cash_account_id" class="form-control">
                                        <option value=''>--Select--</option>
										<?php foreach($account_details as $key=>$account){ 
										if($cash_account_id==$account['id']){ $selected = 'selected'; }else{ $selected = ''; } ?>
											<option value='<?php echo $account['id']; ?>' <?php echo $selected; ?>><?php echo $account['holder'].' ('.$account['acn'].')'; ?></option>
                                        <?php } ?>
                                    </select>
								</div>
                            </div>
							...
							<div class="form-group row">
                                <label class="col-sm-3 col-form-label"
                                       for="pay_cat"><?php echo $this->lang->line('Others') ?></label>

                                <div class="col-sm-9">
                                    <select name="other_account_id" class="form-control">
                                        <option value=''>--Select--</option>
										<?php foreach($account_details as $key=>$account){ 
										if($other_account_id==$account['id']){ $selected = 'selected'; }else{ $selected = ''; }
										?>
											<option value='<?php echo $account['id']; ?>' <?php echo $selected; ?>><?php echo $account['holder'].' ('.$account['acn'].')'; ?></option>
                                        <?php } ?>
                                    </select>
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-sm-4">								
									...
									<input type="hidden" value="customers/link_account" id="action-url">                                   
                                </div>
                            </div>
                        </form>
                    </div>
