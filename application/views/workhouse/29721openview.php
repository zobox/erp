...
<h4 class="card-title col-md-4" style="margin-left: 0px;"> Job Card		<input type="number" name="quantity" id="qty-3" class="form-control" style="display: inline-block;margin-left: 20px;width: 40%;margin-right: 20px;" value="<?=$request_id?>" readonly="">			</h4>
    ...
    <div class="card-content">
      ...
      <div class="card-body">
		...
		<?php if($this->aauth->get_user()->roleid==11 || $this->aauth->get_user()->roleid==5 || $this->aauth->get_user()->roleid==13){ ?>
		<div class="form-group row">
			<div class="col-md-4">
				<div class="row">
					...
					<div class="col-md-7">
						<select class="form-control" name="final_condition" id="final_condition">
							...
							<?php foreach($varients as $key2=>$varient){ ?>
								<option <?php if($product_info->final_condition==$varient->pid){ ?> selected='selected' <?php } ?> value="<?php echo $varient->pid; ?>"><?php echo $varient->product_name; ?></option>
							<?php } ?>			
						</select>
					</div>
				</div>
			</div>
			<?php }else if($product_info->type==1){ ?>
			<div class="col-md-4">
				<div class="row">
					<div class="col-md-5">Final Condition</div>
					<div class="col-md-7">
						<select class="form-control" name="final_condition2" id="final_condition2" disabled>
							<option value="" selected="selected">--Select--</option>
							<?php foreach($varients as $key2=>$varient){ ?>
								<option <?php if($product_info->final_condition==$varient->pid){ ?> selected='selected' <?php } ?> value="<?php echo $varient->pid; ?>"><?php echo $varient->product_name; ?></option>
							<?php } ?>			
						</select>
						<input type="hidden" name="final_condition" id="final_condition" value="<?php echo $product_info->final_condition; ?>">
					</div>
				</div>
			</div>			
			<?php } ?>
