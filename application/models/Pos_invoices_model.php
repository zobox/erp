...
class Pos_invoices_model extends CI_Model
{
    ...
    public function invoice_products($id)
    {
        ...
        if($query->num_rows()>0)
        {
            foreach($query->result() as $key=>$row)
            {
                ...
                $row->product_info = $this->invoiceProductInfo($row->pid);
    ...
    public function invoiceProductInfo($pid)
    {

        $this->db->select('b.product_name,c.name as unit_name,a.pcat,a.sub,a.vb,a.vc,a.pid');
            $this->db->from('geopos_products as a');
            $this->db->join('geopos_products as b','a.sub=b.pid','left');
            ...
            if($query->num_rows()>0)
            {
                foreach ($query->result() as $key => $value) {
                    $rel_id=$value->pcat;
                    $value->pcat = $this->GetParentCatTitleById($rel_id);
                    if($value->sub<1 && $value->vb<1 && $value->vc<1)
                    {
                        
                        $value->product_name = $this->getParentPro($value->pid);
                    }
                    else
                    {
    ...
    public function getParentPro($pid)
    {
