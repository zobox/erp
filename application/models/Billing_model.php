...
class Billing_model extends CI_Model
{
    ...
    public function paynow($tid, $amount, $note, $pmethod, $loc = false,$bill_date=null,$account_d=0,$trans_type='',$pmethod_id='',$insertedmetaid='')
    {
        $account['id'] = false;
        if ($loc) {
        ...
        $this->db->select('geopos_invoices.*,tbl_customers.name,tbl_customers.id AS cid');
        ...
        $data = array(
			...
			'debit' => $amount,
            'payer' => $invoice['name'],
            'payerid' => $invoice['csd2'],
            'method' => $pmethod,
            'method_id' => $pmethod_id,
            'date' => $bill_date,
            'eid' => $invoice['eid'],
            'tid' => $insertedmetaid,
            'note' => $note,
