...
class Purchase_model extends CI_Model
{
    ...
    private function _get_datatables_query()
    {
        ...
        $this->db->select('geopos_purchase.id,geopos_purchase.tid,geopos_purchase.invoicedate,geopos_purchase.invoiceduedate,geopos_purchase.total,geopos_purchase.status,geopos_purchase.items,geopos_purchase.pending_qty,geopos_supplier.name,geopos_purchase.type');
		$this->db->from($this->table);
        $this->db->join('geopos_supplier', 'geopos_purchase.csd=geopos_supplier.id', 'left');
            if ($this->aauth->get_user()->loc) {
            $this->db->where('geopos_purchase.loc', $this->aauth->get_user()->loc);
   ...
   {  
    ...
    {  
        //$data = array();
        $this->db->select("a.qty,a.id,b.warehouse_product_code,b.pid as product_id,c.name as colour_name,d.name as condition_type,u.name as unit_name, bb.pid as color_pid,bb.product_name,bb.vb as varient_id,bb.colour_id,brand.title as brand_name");
        $this->db->from("geopos_purchase_items as a");
        $this->db->join("geopos_products as b","a.pid=b.pid",'left');
        $this->db->join("geopos_products as bb",'bb.sub=b.pid','left');
        $this->db->join("geopos_brand as brand",'bb.b_id=brand.id','left');
	...
	public function getComponentByPid($pid,$vid='',$colour_pid=''){
		...
		if($colour_pid){
			$this->db->or_where("a.product_id",$colour_pid);
		}elseif($vid){
		...
		$this->db->order_by('a.component_name','ASC');
	...
	public function getPurchaseItemsByID($id){
		$this->db->select('b.*,c.product_name,c.warehouse_product_code');		
		$this->db->from("geopos_purchase as a");		
		$this->db->join('geopos_purchase_items as b', 'a.id = b.tid', 'LEFT');
