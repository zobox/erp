...
class Settings_model extends CI_Model
{
    ...
    public function update_company($id, $name, $phone, $email, $address, $city, $region, $country, $postbox, $taxid,$data_share,$foundation)
    {
		...
		$company_mail = $this->input->post('company_mail', true);
		$company_phone_no = $this->input->post('company_phone_no', true);
		$pan = $this->input->post('pan', true);
		$tan = $this->input->post('tan', true);
		$cin = $this->input->post('cin', true);
		$gst = $this->input->post('gst', true);
		$establishment_code = $this->input->post('establishment_code', true);
		$esi_no = $this->input->post('esi_no', true);
		$pf_no = $this->input->post('pf_no', true);
		$iec_code = $this->input->post('iec_code', true);
	...
	public function saveAgencyCommission(){
		...
		$this->db->where('id',$id);		
		if ($this->db->update('geopos_agency_commission', $data)) {						
			$this->aauth->applog("[Update Agency Commission] -$commission ID " . $id, $this->aauth->get_user()->username);
			echo json_encode(array('status' => 'Success', 'message' =>
                        $this->lang->line('UPDATED') . "<a href='" . base_url('settings/agencycommission?id=1') . "' class='btn btn-grey-blue btn-lg'><span class='fa fa-list-alt' aria-hidden='true'></span>  </a>"));
		} else {
			echo json_encode(array('status' => 'Error', 'message' =>
                        $this->lang->line('ERROR')));
		}
	...
	public function getAgencyCommission($id){		
		$this->db->where('id',$id);
		$query = $this->db->get("geopos_agency_commission");		
		$data = array();
		if ($query->num_rows() > 0) {
	...
	public function saveTds(){
		...
		$this->db->where('id',$id);		
		if ($this->db->update('geopos_tds', $data)) {						
			$this->aauth->applog("[Update TDS] -$tds ID " . $id, $this->aauth->get_user()->username);
			echo json_encode(array('status' => 'Success', 'message' =>
                        $this->lang->line('UPDATED') . "<a href='" . base_url('settings/tds?id=1') . "' class='btn btn-grey-blue btn-lg'><span class='fa fa-list-alt' aria-hidden='true'></span>  </a>"));
		} else {
			echo json_encode(array('status' => 'Error', 'message' =>
                        $this->lang->line('ERROR')));
		}
	...
	public function saveBankCharges(){
		...
		$this->db->where('id',$id);		
		if ($this->db->update('geopos_bank_charges', $data)) {						
			$this->aauth->applog("[Update Bank Charges] -$charges ID " . $id, $this->aauth->get_user()->username);
			echo json_encode(array('status' => 'Success', 'message' =>
                        $this->lang->line('UPDATED') . "<a href='" . base_url('settings/bank_charges?id=1') . "' class='btn btn-grey-blue btn-lg'><span class='fa fa-list-alt' aria-hidden='true'></span>  </a>"));
		} else {
			echo json_encode(array('status' => 'Error', 'message' =>
                        $this->lang->line('ERROR')));
		}
	...
	public function getBankCharges($id){		
		$this->db->where('id',$id);
		$query = $this->db->get("geopos_bank_charges");		
		$data = array();
		if ($query->num_rows() > 0) {
			foreach ($query->result() as $key=>$row) {									
				$data = $row;
