...
class Dashboard_model extends CI_Model
{
    ...
    public function recentInvoices()
    {
        ...
        $query = $this->db->query("SELECT i.id,i.type,i.pmethod_id,i.tid,i.invoicedate,i.total,i.status,i.i_class,c.name,c.picture,i.csd
		FROM geopos_invoices AS i LEFT JOIN geopos_customers AS c ON i.csd=c.id where 1 and i.type!=4 $whr ORDER BY i.id DESC LIMIT 10"); 
        return $query->result_array();
        ...
        public function recentBuyers()
    {
        ...
        //echo "SELECT MAX(i.id) AS iid,i.csd,SUM(i.total) AS total, c.cid,MAX(c.picture) as picture ,MAX(c.name) as name,MAX(i.status) as status FROM geopos_invoices AS i LEFT JOIN (SELECT geopos_customers.id AS cid, geopos_customers.picture AS picture, geopos_customers.name AS name FROM geopos_customers) AS c ON c.cid=i.csd $whr GROUP BY i.csd ORDER BY iid DESC LIMIT 10;"; die;
        //echo "select i.id,i.total,b.name from geopos_invoices as i left join tbl_customers as b on i.csd2.b.id $whr and i.type=2 group by i.csd2 order by i.id desc"; die;
        $sql = "select i.id,i.total,b.name from geopos_invoices as i left join tbl_customers as b on i.csd2.b.id $whr and i.type=2 group by i.csd2 order by i.id desc";
