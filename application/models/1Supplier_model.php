...
{
	private $path = 'userfiles/documents/supplier/';
    ...
    public function add($name, $company, $phone, $email, $address_b, $city_b, $state_b, $country_b, $postbox_b, $address_s, $city_s, $state_s, $country_s, $postbox_s, $pan, $gst, $rtgs, $account_no, $bank_name, $branch_name, $pan_up, $gst_up, $cancelled_cheque_up,$same_as_billing)
    {
		$data = array(
            'name' => $name,
            'company' => $company,
			...
			'pan_up' => $pan_up,
            'gst_up' => $gst_up,
            'cancelled_cheque_up' => $cancelled_cheque_up,
        ...
        if ($this->db->insert('geopos_supplier', $data)) {		  
            ...
            //echo json_encode(array('status' => 'Success', 'message' =>
               // $this->lang->line('ADDED') . "  <a href='create' class='btn btn-blue btn-lg'><span class='fa fa-plus-circle' aria-hidden='true'></span>  </a> <a href='" . base_url('supplier') . "' class='btn btn-grey-blue btn-lg'><span class='fa fa-list-alt' aria-hidden='true'></span>  </a>"));
        } else {
            header('location:'.base_url().'/supplier');
    ...
    public function edit($id, $name, $company, $phone, $email, $address_b, $city_b, $state_b, $country_b, $postbox_b, $address_s, $city_s, $state_s, $country_s, $postbox_s, $pan, $gst, $rtgs, $account_no, $bank_name, $branch_name, $pan_up, $gst_up, $cancelled_cheque_up,$same_as_billing)
    {
        $data = array(
            'name' => $name,
            'company' => $company,
        ...
        if ($this->db->update('geopos_supplier')) {
            header('location:'.base_url().'/supplier');
            //echo json_encode(array('status' => 'Success', 'message' =>
                //$this->lang->line('UPDATED') . " <a href='" . base_url('supplier/edit?id=' . $id) . "' class='btn btn-blue btn-lg'><span class='fa fa-eye' aria-hidden='true'></span>  </a> <a href='" . base_url('supplier') . "' class='btn btn-grey-blue btn-lg'><span class='fa fa-list-alt' aria-hidden='true'></span>  </a>"));
        } else {
            header('location:'.base_url().'/supplier');
            //echo json_encode(array('status' => 'Error', 'message' =>
               // $this->lang->line('ERROR')));
	...
	function upload($fieldname)
	{
		...
		$config['allowed_types'] = 'jpg|png|jpeg|gif|doc|docx|pdf|xls|xlsx|txt';
		$config['max_size'] = '2000';
		$config['remove_spaces'] = true;
		$config['overwrite'] = false;
		$config['encrypt_name'] = true;
