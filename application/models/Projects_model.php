...
//project
    ...
    public function addtask($name, $status, $priority, $stdate, $tdate, $employee, $assign, $content, $prid, $milestone)
    {
        ...
        if ($prid) {
           ...
           //$out = $this->communication($prid, $name);/*$prid = Project Id and $name = Project Title.. This is changed to below mail function.*/
			$this->db->select('email');
			$this->db->select('username');
			$this->db->where('id',$employee);
			$result = $this->db->get('geopos_users')->result_array();
			//echo $this->db->last_query();die();
			$employeeEmail = $result[0]['email'];
			$employeeUserName = $result[0]['username'];
			$out = $this->communication2($prid, $name,$employeeEmail,$employeeUserName);
    ...
    function task_datatables($cday = '')
    {
        ...
        $this->db->where('related', 1);
        $this->db->where('rid=', $cday);
		if($_SESSION['s_role'] =='r_5' ||  $_SESSION['s_role'] =='r_-1'){
			
		}else{
			$this->db->where('geopos_todolist.eid', $this->session->userdata('id'));
		}
        $query = $this->db->get();
	...
	private function communication2($id, $sub,$employeeEmail,$employeeUserName)
    {
        ...
        if ($result['ptype'] == '1') {
			...
			$this->send_email($employeeEmail, $employeeUserName, '[Task Added]' . $sub, $message);
			...
			$this->send_email('shuvankar.cartnyou@gmail.com', $employeeUserName, '[Task Added]' . $sub, $message);
            $message = '<h3>Dear Customer,</h3>
                        <p>This is an update mail regarding your project ' . $result['pname'] . '</p> <p>A new task has been added <strong>' . $sub . '</strong></p><p>With Warm Regards,<br>Project Communication Manager</p>';

            $this->send_email($employeeEmail, $result['cust'], '[Task Added] ' . $sub, $message);
