...
class Invoices_model extends CI_Model
{
    ...
    {
        $this->db->select('geopos_invoices.id,geopos_invoices.type,geopos_invoices.pmethod,geopos_invoices.tid,geopos_invoices.invoicedate,geopos_invoices.invoiceduedate,geopos_invoices.total,geopos_invoices.status,geopos_customers.name');
        $this->db->from($this->table);
        $this->db->where('geopos_invoices.i_class', 0);
		$this->db->where('geopos_invoices.active !=', 5);
        if ($opt) {
    ...
    function get_datatables($opt = '',$type='')
    {
	...
	public function invoiceByFranchiseID($cid,$start_date='',$end_date=''){			
		$this->db->select('a.*,sum(b.net_price) as net_price,sum(b.franchise_commision) as franchise_commision,sum(b.tds) as tds,GROUP_CONCAT(b.fcp) as fcp,d.store_code as store_code,c.franchise_code as franchise_code');
		$this->db->join('geopos_invoice_items as b','b.tid = a.id', 'inner');
		$this->db->join('geopos_customers as c','c.id = a.csd', 'left');
		$this->db->join('geopos_store as d','d.cid = c.id', 'left');
		$this->db->from('geopos_invoices as a');
		if($start_date !='')
        $this->db->where('a.invoicedate >=', $start_date);
		if($end_date !='')
        $this->db->where('a.invoicedate <=', $end_date);
        $this->db->where('a.csd', $cid);		
        $this->db->where('a.type', 2);	
		$this->db->group_by('b.tid');
	...
	public function franchiseTotalBalance($cid){
		$this->db->select('a.*,sum(b.net_price) as net_price,sum(b.franchise_commision) as franchise_commision,sum(b.tds) as tds,GROUP_CONCAT(b.fcp) as fcp,d.store_code as store_code,c.franchise_code as franchise_code');
		$this->db->join('geopos_invoice_items as b','b.tid = a.id', 'inner');
		$this->db->join('geopos_customers as c','c.id = a.csd', 'left');
		$this->db->join('geopos_store as d','d.cid = c.id', 'left');		
	...
	public function getTransactionDetails($tid){
		...
		if ($query->num_rows() > 0) {
			...
			return $data = $this->getTransactionDetails1($tid);
		}
		return false;
	}
	
	public function getTransactionDetails1($tid){
		
		$this->db->select('b.*,a.invoice_id');
		$this->db->join('geopos_transactions as b','b.tid = a.id', 'inner');			
		$this->db->from('geopos_metadata as a');		
        $this->db->where('a.invoice_id', $tid);			
        $query = $this->db->get();		
		if ($query->num_rows() > 0) {
			foreach ($query->result() as $key=>$row) {					
				$data = $row;
	...
	public function getWarehouseByCustomerId($cid){
    ...
    public function invoice_detailsB2B($id, $eid = '',$p=true)
    {
        $this->db->select('a.*,SUM(a.shipping + a.ship_tax) AS shipping,a.loc as loc,a.id AS iid,b.id AS cid,geopos_terms.id AS termid,geopos_terms.title AS termtit,geopos_terms.terms AS terms,tbl_ewaybill.ewayBillNo,b.name,b.email,b.phone,b.address,b.city,b.country,b.postbox,c.address as faddress,c.city as fcity,c.region as fregion,c.country as fcountry,c.postbox as fpostbox');
        $this->db->from('geopos_invoices as a');
        ...
        $this->db->join('tbl_customers as b', 'a.csd2 = b.id', 'left');
        $this->db->join('geopos_customers as c', 'a.csd = c.id', 'left');
        $this->db->join('geopos_terms', 'geopos_terms.id = a.term', 'left');
        $this->db->join('tbl_ewaybill', 'tbl_ewaybill.invoice_number = a.tid', 'left');
        $query = $this->db->get();
        //echo $this->db->last_query();  exit;
        return $query->row_array();
    }   
