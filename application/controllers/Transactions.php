...
class Transactions extends CI_Controller
{
     ...
     public function translist()
    {
        ...
        foreach ($list as $prd) {
            ...
            $row[] = '<a href="' . base_url() . 'transactions/view?id=' . $pid . '" class="btn btn-success btn-sm"><span class="fa fa-eye"></span>  ' . $this->lang->line('View') . '</a> <a href="' . base_url() . 'transactions/print_t?id=' . $pid . '" class="btn btn-info btn-sm"  title="Print"><span class="fa fa-print"></span>  ' . $this->lang->line('Print') . '</a>&nbsp;<a  href="#" data-object-id="' . $pid . '" class="btn btn-danger btn-sm delete-object"><span class="fa fa-trash"></span></a>';
            $data[] = $row;
        }
    ...
    public function view()
    {
        ...
        if ($data['trans']['acid'] > 0) {
            $data['cdata'] = $this->transactions->cview($data['trans']['acid'], $data['trans']['ext']);
        } else {
    ...
    public function asset_purchase_payment()
    {
        ...
        $asset_order_id = $this->input->post('asset_order_id');
        $tid = $this->input->post('tid', true);
        $amount = $this->input->post('amount', true);
        $paydate = $this->input->post('paydate', true);
        $note = $this->input->post('shortnote', true);
        $pmethod = $this->input->post('pmethod', true);
        $acc_name = $this->input->post('fran_name',true);
        $acid = $this->input->post('sid', true);
        $cid = $this->input->post('cid', true);
        $cname = $this->input->post('cname', true);
        $paydate = datefordatabase($paydate);
        /*$this->db->select('holder');
        ...
        $query = $this->db->get();
        $account = $query->row_array();*/
        $invoicedate = $this->input->post('paydate');
        ...
        {
            echo json_encode(array('status' => 'Failed', 'message' => 'Franchise Has Not sufficient Amount To Pay For This Asset Order', 'pstatus' => $this->lang->line($status), 'activity' => $activitym, 'amt' => $totalrm, 'ttlpaid' => $paid_amount));
        ...
        else
        {
         ...
         $data = array('type' => 21,
            'rid' => $customer_id,
            'col1' => $amount,
            'col2' => date('Y-m-d H:i:s') .'  ASSET #'.$tid. '(Asset Order) Sale to Franchise by ' . $this->aauth->get_user()->username,
            'invoice_id' => $asset_order_id,
            'invoice_url' => $invoice_detail,
            'd_date' => $invoicedate);
