...
class Quote extends CI_Controller
{
	...
	public function qc_data()
    {
        ...
        $head['title'] = "QC Data Import";
        $data['eid'] = intval($this->input->get('eid'));
        $head['usernm'] = $this->aauth->get_user()->username;
        $this->load->view('fixed/header', $head);
        $this->load->view('quotes/qc-data', $data);
        $this->load->view('fixed/footer');
	}
	
	public function qc_data_excel()
    {
        $head['title'] = "QC Data Import";
        $data['eid'] = intval($this->input->get('eid'));
        $head['usernm'] = $this->aauth->get_user()->username;
        $this->load->view('fixed/header', $head);
        $this->load->view('quotes/qc-data-excel', $data);
        $this->load->view('fixed/footer');
    }
	
	
	public function ajax_qc_data()
    {
        $eid = 0;
        if ($this->aauth->premission(9)) {
            $eid = $this->input->post('eid');
        }
		$excel_sheet_id = $this->input->post('id');
        $list = $this->quote->get_qc_data($excel_sheet_id);
        $data = array();
        $no = $this->input->post('start');
        foreach ($list as $qc) {
            $no++;
            ...
            $row[] = $qc->sticker_no;
            $row[] = $qc->brand;
            $row[] = $qc->product_label_name;
            $row[] = $qc->varient;
            $row[] = $qc->colour;
            $row[] = $qc->product_condition;
            $row[] = $qc->imei1;
            $row[] = $qc->imei2;
            $row[] = $qc->item_replaced;
            $row[] = $qc->qc_engineer;
            $data[] = $row;
        }
	...
	public function ajax_qc_data_excel(){
		$eid = 0;
        if ($this->aauth->premission(9)) {
            $eid = $this->input->post('eid');
        }
        $list = $this->quote->get_qc_data_excel();
        $data = array();
        $no = $this->input->post('start');
        foreach ($list as $qc) {
			$no++;
            ...
            $row[] = $qc->date_created;
            $row[] = $qc->name;
            $row[] = '<a href="' . base_url("quote/qc_data_excel?id=$qc->id") . '" class="btn btn-primary btn-xs">' . $this->lang->line('View') . '</a>';;            
            $data[] = $row;
        }

        $output = array(
            "draw" => $_POST['draw'],
			"recordsTotal" => $this->quote->count_all_qc_data_excel($eid),
			"recordsFiltered" => $this->quote->count_all_qc_data_excel($eid),
            "data" => $data,
        );
        //output to json format
        echo json_encode($output);
	...
	public function sample_qc(){
		
		$fileName = 'sample_qc.xlsx';  
		$objPHPExcel = new PHPExcel();
		$objPHPExcel->setActiveSheetIndex(0);
		//$objPHPExcel->getActiveSheet()->mergeCells('A1:J1');
		//$objPHPExcel->getActiveSheet()->SetCellValue('A1', 'Leads - ');
        $objPHPExcel->getActiveSheet()->SetCellValue('A1', 'Enter Sticker #');
        $objPHPExcel->getActiveSheet()->SetCellValue('B1', 'Brand');
        $objPHPExcel->getActiveSheet()->SetCellValue('C1', 'Mobile Model');
        $objPHPExcel->getActiveSheet()->SetCellValue('D1', 'Varient');
        $objPHPExcel->getActiveSheet()->SetCellValue('E1', 'Colour');
        $objPHPExcel->getActiveSheet()->SetCellValue('F1', 'Enter Product Condition');
        $objPHPExcel->getActiveSheet()->SetCellValue('G1', 'Last 4 Digits');
        $objPHPExcel->getActiveSheet()->SetCellValue('H1', 'Last 4 Digits');
        $objPHPExcel->getActiveSheet()->SetCellValue('I1', 'Write all Items to be Replaced');
        $objPHPExcel->getActiveSheet()->SetCellValue('J1', 'Qc Engineer Name');
		
		$objPHPExcel->getActiveSheet()->SetCellValue('A2', '#');
        $objPHPExcel->getActiveSheet()->SetCellValue('B2', 'Brand');
        $objPHPExcel->getActiveSheet()->SetCellValue('C2', 'Product Label Name');
        $objPHPExcel->getActiveSheet()->SetCellValue('D2', 'Varient');
        $objPHPExcel->getActiveSheet()->SetCellValue('E2', 'Colour');
        $objPHPExcel->getActiveSheet()->SetCellValue('F2', 'Product Condition');	
        $objPHPExcel->getActiveSheet()->SetCellValue('G2', 'IMEI-1');	
        $objPHPExcel->getActiveSheet()->SetCellValue('H2', 'IMEI-2');	
        $objPHPExcel->getActiveSheet()->SetCellValue('I2', 'Items to be Replaced');	
        $objPHPExcel->getActiveSheet()->SetCellValue('J2', 'Qc Engineer');	
	...
	public function import_qc(){	
		...
		{		
			...
			$inserted_id = $this->quote->insertQcExcel(); 			
			foreach($object->getWorksheetIterator() as $worksheet)
			{
				...
				{		
					$sticker_no = $worksheet->getCellByColumnAndRow(0, $row)->getValue();
					$brand = $worksheet->getCellByColumnAndRow(1, $row)->getValue();
					$product_label_name = $worksheet->getCellByColumnAndRow(2, $row)->getValue();
					$varient = $worksheet->getCellByColumnAndRow(3, $row)->getValue();
					$colour = $worksheet->getCellByColumnAndRow(4, $row)->getValue();
					$product_condition	= $worksheet->getCellByColumnAndRow(5, $row)->getValue();
					$imei1	= $worksheet->getCellByColumnAndRow(6, $row)->getValue();
					$imei2	= $worksheet->getCellByColumnAndRow(7, $row)->getValue();
					$item_replaced	= $worksheet->getCellByColumnAndRow(8, $row)->getValue();
					$qc_engineer	= $worksheet->getCellByColumnAndRow(9, $row)->getValue();
					
					$data['excel_sheet_id'] = $inserted_id;
					$data['sticker_no'] = $sticker_no;
					$data['brand'] = $brand;
					$data['product_label_name'] = $product_label_name;
					$data['varient'] = $varient;
					$data['colour'] = $colour;				
					$data['product_condition'] = $product_condition;
					$data['imei1'] = $imei1;
					$data['imei2'] = $imei2;					
					$data['item_replaced'] = $item_replaced;
			...
			$this->quote->import_qc($data1); 
			redirect('/quote/qc_data', 'refresh');			
		}else{
			 echo json_encode(array('status' => 'Error', 'message' =>
				$this->lang->line('ERROR')));
		}
