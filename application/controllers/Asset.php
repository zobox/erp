...
class Asset extends CI_Controller
{
     ...
     public function add()
    {
      $data['cat'] = $this->asset->category_list();
        ...
        $data['autogenerate'] = $this->asset->lastAssetId();
        $this->load->model('units_model', 'units');
  ...
  public function add_asset()
     ...
     public function ajax_list()
    {
        ...
        foreach ($list as $invoices) {
            ...
            $row = array();
            $row[] = $no;
            $row[] = 'ASSET_PO#ZO'.$invoices->tid;
            $row[] = '<a href="' . base_url("supplier/view?id=$invoices->supplier_id") . '" target="_blank">'.$invoices->company.'</a>';
            $row[] = dateformat($invoices->invoicedate);
            $row[] = amountExchange($invoices->total, 0, $this->aauth->get_user()->loc);
            $row[] = '<span class="st-' . $invoices->status . '">' . $this->lang->line(ucwords($invoices->status)) . '</span>';
            $row[] = '<a href="' . base_url("asset/view?id=$invoices->id") . '" class="btn btn-success btn-xs"><i class="fa fa-eye"></i> ' . $this->lang->line('View') . '</a> &nbsp; <a href="' . base_url("asset/printinvoice?id=$invoices->id") . '&d=1" class="btn btn-info btn-xs"  title="Download"><span class="fa fa-download"></span></a>';
    ...
    public function create_asset()
    {
        ...
        $data['warehouse'] = $this->categories_model->warehouse_list2();
        $data['taxlist'] = $this->common->taxlist();
    ...
    public function asset_list()
    {
        ...
        foreach ($list as $prd) {
            ...
            &nbsp;<a href="' . base_url() . 'asset/edit?id=' . $pid . '"  class="btn btn-purple btn-sm"><span class="fa fa-edit"></span>' . $this->lang->line('Edit') . '</a>
                                </div>';
            $data[] = $row;
        } 
        $output = array(
      ...
      public function edit()
    {
      ...
      $common = $this->load->library("Common"); 
        ...
        $query = $this->db->get();
        $data['product'] = $query->row_array();
        $data['product_category_array'] = array_reverse(explode("-",substr($this->asset->getParentcategory($data['product']['pcat']),1)));

        $data['product_category_array_title'] = $this->asset->getCategoryNames($data['product_category_array']);
        $data['brand'] = $this->asset->brand_list();
        $data['warehouse'] = $this->categories_model->warehouse_list2();
        $data['units'] = $this->asset->units();
        $data['cat_ware'] = $this->asset->cat_ware($pid);
        $data['cat_sub'] = $this->asset->sub_cat_curr($data['product']['sub_id']);
        $data['cat_sub_list'] = $this->asset->sub_cat_list($data['product']['pcat']);
        $data['cat'] = $this->asset->category_list();
    ...
    public function asset_order_edit()
    {
       ...
       $data['taxlist'] = $this->common->taxlist($this->config->item('tax'));
        ...
        $data['id'] = $tid;
          ...
          $this->db->select('w.*,p.total,p.po_type,c.name as cust_name,s.name as supp_name,s.phone as supp_phone,w.sid,w.cid as customer_id,s.address as supp_address,c.franchise_code,s.city as supp_city,s.country as supp_country,s.email as supp_email,p.tid as invoice_id,p.refer,p.invoicedate,p.invoiceduedate,p.format_discount,p.notes,p.tax as ptax,p.discount as pdis,p.shipping,p.subtotal as psubtotal,p.term,p.ship_tax,p.ship_tax_type,p.taxstatus,gw.title as warehouse_title');
        $this->db->from('tbl_asset_warehouse as w');
        $this->db->join('tbl_asset_purchase as p',"w.tid=p.id",'left');
        $this->db->join('geopos_customers as c',"w.cid=c.id",'left');
        $this->db->join('geopos_warehouse as gw',"w.wid=gw.id",'left');
        $this->db->join('geopos_supplier as s',"w.sid=s.id",'left');
