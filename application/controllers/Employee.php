...
class Employee extends CI_Controller
{
    ...
    {	
        ...
        if ((string)$this->aauth->get_user($a)->id != $this->aauth->get_user()->id) {
            ...
            if ($nuid > 0) {
                ...
                $this->employee->add_employee($nuid, (string)$this->aauth->get_user($a)->username, $name, $roleid, $franchise_id, $phone, $address, $city, $region, $country, $postbox, $location, $salary, $commission, $department);
    ...
    public function update()
    {
        ...
        if ($this->input->post()) {
            ...
            $this->employee->update_employee($eid, $name, $phone, $phonealt, $address, $city, $region, $country, $postbox, $location, $salary, $department, $commission, $roleid,$franchise_id);
	...
	public function permissions_update()
    {
        ...
        foreach ($permission as $row) {
            ...
            if ($this->input->post($name14)) $val14 = 1;
            if ($this->input->post($name15)) $val15 = 1;
            if ($this->input->post($name16)) $val16 = 1;
            
            if ($this->aauth->get_user()->roleid == 5 && $i == 9) $val5 = 1;
            $data = array('r_1' => $val1, 'r_2' => $val2, 'r_3' => $val3, 'r_4' => $val4, 'r_5' => $val5, 'r_6' => $val6, 'r_7' => $val7, 'r_9' => $val9, 'r_10' => $val10, 'r_11' => $val11, 'r_12' => $val12, 'r_13' => $val13, 'r_14' => $val14, 'r_15' => $val15, 'r_16' => $val16);
            $this->db->set($data);
            $this->db->where('id', $i);
