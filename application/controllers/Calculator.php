...
class Calculator extends CI_Controller
{
	...
	public function calculate(){
		...
		$zo_bulk_discount = $this->input->post('zo_bulk_discount',true);
		$zo_bulk_discount_value = $this->input->post('zo_bulk_discount_value',true);
		$zo_bulk_margin = $this->input->post('zo_bulk_margin',true);
		$zo_bulk_margin_value = $this->input->post('zo_bulk_margin_value',true);		
		...
		$pnl_retail = $hindizo_sale_price;
		$pnl_obc = $hindizo_sale_price;
		$pnl_zobulk = $hindizo_sale_price;
		
		if($obc_discount==2){				
			$obc_discount_actual = (($hindizo_sale_price*$obc_discount_value)/100);
			$pnl_obc =  $pnl_obc - $obc_discount_actual;	
			$obc_selling_price = $hindizo_sale_price-$obc_discount_actual;
		}else if($obc_discount==1){
			$pnl_obc =  $pnl_obc - $obc_discount_value;
			$obc_selling_price = $hindizo_sale_price-$obc_discount_value;
			$obc_discount_actual = $obc_discount;
		}
		
		if($zo_bulk_discount==2){				
			$zo_bulk_discount_actual = (($hindizo_sale_price*$zo_bulk_discount_value)/100);
			$pnl_zobulk =  $pnl_zobulk - $zo_bulk_discount_actual;	
			$zo_bulk_selling_price_after_zbd = $hindizo_sale_price-$zo_bulk_discount_actual;
		}else if($zo_bulk_discount==1){
			$pnl_zobulk =  $pnl_zobulk - $zo_bulk_discount_value;
			$zo_bulk_selling_price_after_zbd = $hindizo_sale_price-$zo_bulk_discount_value;
			$obc_discount_actual = $zo_bulk_discount;
		}
		
		if($zo_bulk_margin==2){				
			$zo_bulk_margin_actual = (($hindizo_sale_price*$zo_bulk_margin_value)/100);
			$pnl_zobulk =  $pnl_zobulk - $zo_bulk_margin_actual;	
			$zo_bulk_selling_price_after_margin = $zo_bulk_selling_price_after_zbd-$zo_bulk_margin_actual;
		}else if($zo_bulk_margin==1){
			$pnl_zobulk =  $pnl_zobulk - $zo_bulk_margin_value;
			$zo_bulk_selling_price_after_margin = $zo_bulk_selling_price_after_zbd-$zo_bulk_margin_value;
			$zo_bulk_margin_actual = $zo_bulk_margin_value;
		}
		...
		if($refurbishment_cost_type==2){
			...
			$pnl_retail =  $pnl_retail - $refurbishment_cost_actual;
			$pnl_obc =  $pnl_obc - $refurbishment_cost_actual;
			$pnl_zobulk =  $pnl_zobulk - $refurbishment_cost_actual;
			//$refurbishment_cost = $refurbishment_cost_actual;
		}else if($refurbishment_cost_type==1){
			$pnl_retail =  $pnl_retail - $refurbishment_cost;
			$pnl_obc =  $pnl_obc - $refurbishment_cost;
			$pnl_zobulk =  $pnl_zobulk - $refurbishment_cost;
			$refurbishment_cost_actual = $refurbishment_cost;
		}
		
		if($packaging_cost_type==2){
			$packaging_cost_actual = (($sale_without_gst_price*$packaging_cost)/100);
			$pnl_retail =  $pnl_retail - $packaging_cost_actual;
			$pnl_obc =  $pnl_obc - $packaging_cost_actual;
			$pnl_zobulk =  $pnl_zobulk - $packaging_cost_actual;
			//$packaging_cost = $packaging_cost_actual;
		}else if($packaging_cost_type==1){
			$pnl_retail =  $pnl_retail - $packaging_cost;
			$pnl_obc =  $pnl_obc - $packaging_cost;
			$pnl_zobulk =  $pnl_zobulk - $packaging_cost;
			$packaging_cost_actual = $packaging_cost;
		}
		
		if($sales_support_type==2){
			$sales_support_actual = (($sale_without_gst_price*$sales_support)/100);
			$pnl_retail =  $pnl_retail - $sales_support_actual;
			$pnl_obc =  $pnl_obc - $sales_support_actual;
			//$sales_support = $sales_support_actual;
		}else if($sales_support_type==1){
			$pnl_retail =  $pnl_retail - $sales_support;
			$pnl_obc =  $pnl_obc - $sales_support;
			$pnl_zobulk =  $pnl_zobulk - $sales_support;
			$sales_support_actual = $sales_support;
		}
		 
		if($promotion_cost_type==2){
			$promotion_cost_actual = (($sale_without_gst_price*$promotion_cost)/100);
			$pnl_retail =  $pnl_retail - $promotion_cost_actual;
			$pnl_obc =  $pnl_obc - $promotion_cost_actual;
			$pnl_zobulk =  $pnl_zobulk - $promotion_cost_actual;
			//$promotion_cost = $promotion_cost_actual;
		}else if($promotion_cost_type==1){
			$pnl_retail =  $pnl_retail - $promotion_cost;
			$pnl_obc =  $pnl_obc - $promotion_cost;
			$pnl_zobulk =  $pnl_zobulk - $promotion_cost;
			$promotion_cost_actual = $promotion_cost;
		}
		
		if($hindizo_infra_type==2){
			$hindizo_infra_actual = (($sale_without_gst_price*$hindizo_infra)/100);
			$pnl_retail =  $pnl_retail - $hindizo_infra_actual;
			$pnl_obc =  $pnl_obc - $hindizo_infra_actual;
			$pnl_zobulk =  $pnl_zobulk - $hindizo_infra_actual;
			//$hindizo_infra = $hindizo_infra_actual;
		}else if($hindizo_infra_type==1){
			$pnl_retail =  $pnl_retail - $hindizo_infra;
			$pnl_obc =  $pnl_obc - $hindizo_infra;
			$pnl_zobulk =  $pnl_zobulk - $hindizo_infra;
			$hindizo_infra_actual = $hindizo_infra_actual;
		}
		...
		if($hindizo_margin_new!=''){
			...
			if($hindizo_margin_type==2){				
				...
				//$pnl_retail =  $pnl_retail - $hindizo_margin_actual;
				//$hindizo_margin = $hindizo_margin_actual;
			}else if($hindizo_margin_type==1){
				//$pnl_retail =  $pnl_retail - $hindizo_margin;
				$hindizo_margin_actual = $hindizo_margin;
			}
			
			if($hindizo_margin_new_actual==$hindizo_margin_actual){
				$pnl_retail =  $pnl_retail - $hindizo_margin_actual;
				$pnl_obc =  $pnl_obc - $hindizo_margin_actual;
				$pnl_zobulk =  $pnl_zobulk - $hindizo_margin_actual;
			}else{
			...
			if($hindizo_margin_type==2){				
				...
				$pnl_retail =  $pnl_retail - $hindizo_margin;
				$pnl_obc =  $pnl_obc - $hindizo_margin;
				$pnl_zobulk =  $pnl_zobulk - $hindizo_margin;
				$hindizo_margin_actual = $hindizo_margin;
			}
		}		
		...
		$pnl_retail =  $pnl_retail - $agency_commission_actual;
		$pnl_obc =  $pnl_obc - $agency_commission_actual;
		$pnl_zobulk =  $pnl_zobulk - $agency_commission_actual;
		...
		$pnl_retail =  $pnl_retail - $bank_charges_actual;
		$pnl_obc =  $pnl_obc - $bank_charges_actual;
		$pnl_zobulk =  $pnl_zobulk - $bank_charges_actual;
		
		//Retail
		$pnl_retail =  $pnl_retail + $gst_val;
		$pnl_retail =  $pnl_retail - $sale_projected_gst;
		$pnl_retail =  $pnl_retail - $without_gst_price;
		
		//Open Box
		$pnl_obc =  $pnl_obc + $gst_val;
		$pnl_obc =  $pnl_obc - $sale_projected_gst;
		$pnl_obc =  $pnl_obc - $without_gst_price;
		
		//Zo Bulk
		$pnl_zobulk =  $pnl_zobulk + $gst_val;
		$pnl_zobulk =  $pnl_zobulk - $sale_projected_gst;
		$pnl_zobulk =  $pnl_zobulk - $without_gst_price;
		
		
		$pnl_retail = ($pnl_retail+$gst_val)-$sale_projected_gst;
		$pnl_obc = ($pnl_obc+$gst_val)-$sale_projected_gst;
		$pnl_zobulk = ($pnl_zobulk+$gst_val)-$sale_projected_gst;
		...
		$return_data['agency_commission_actual'] = $agency_commission_actual;		
		...
		$return_data['pnl_retail'] = $pnl_retail;	
		$return_data['pnl_obc'] = $pnl_obc;	
		$return_data['pnl_zobulk'] = $pnl_zobulk;	
		$return_data['obc_selling_price'] = $obc_selling_price;	
		$return_data['zo_bulk_selling_price'] = $zo_bulk_selling_price_after_margin;	
