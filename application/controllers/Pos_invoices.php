...
class Pos_invoices extends CI_Controller
{
    ...
    public function create()
    {
        ...
        $this->db->set('hold_status',0);
        $this->db->where('twid',$data['swarehouse']->id);
		...
		$data['franchise'] = $this->customers->getFranchchiseBySalesPerson();
    ...
    public function action()
    {
        ...
        $wid = $this->input->post('warehouses', true);
		$warehouse = $this->products_cat->otherwarehouse($wid);		
        $coupon_amount = 0;
        $coupon_n = '';
        ...
        if ($ptype == 4) {
            ...
            $pmethod_id = $this->input->post('p_method');
			switch($pmethod_id){
				case 1: $pmethod = 'Cash';
				break;
				case 2: $pmethod = 'Bank';
				...
				case 3: $pmethod = 'UPI';
				break;
				case 4: $pmethod = 'Rupay Debit Card';
				break;
				case 5: $pmethod = 'Debit Card (Master Visa Maestro)';
				break;
				case 6: $pmethod = 'Credit card';
				break;
				case 7: $pmethod = 'EMI Charges';
				break;
			}			
			...
			switch($pmethod_id){
				case 1 : $payment_charge = 0;
						break; 
				case 2 : $payment_charge = 0;
						break; 
				case 3 : $payment_charge = 0;
						break; 
				case 4 : $payment_charge = 0;
						break; 
				case 5 : if($total<2000){ $payment_charge = ($total*0.40)/100; }else if($total>=2000){ $payment_charge = ($total*0.90)/100; }
						break; 
				case 6 : $payment_charge = ($total*1.99)/100;
						break; 
				case 7 : $payment_charge = ($total*2.99)/100;
						break; 
			}
			
			$franchise_dtl = $this->customers->getFranchiseByCustromerID($warehouse[0]->cid);
            ...
            if ($this->db->insert('geopos_invoices', $data)) {
                ...
                $marginal_gst_price = $this->input->post('marginal_gst_price');
                $marginal_product_type = $this->input->post('marginal_product_type');
                ...
                $product_serial_id = $this->input->post('serial_id');
				$this->db->select('*');
				$this->db->from('geopos_bank_charges');
				$get_charge = $this->db->get();
				$bank_charges = $get_charge->result_array();				
                ...
                if(is_array($pid)) {
                    foreach ($pid as $key => $value) {
						...
						$fcp = $this->products->getFranchiseCommissionByProductID($franchise->module,$product_id[$key],$purpose='2',$franchise->franchise_id,$commissiontype='retail_commision_percentage'); 
                        ...
                        $data = array(
                            ...
                            'marginal_gst_price' => $marginal_gst_price[$key],
                            'marginal_product_type' => $marginal_product_type[$key],
						...
						//if ($product_id[$key] > 0 and $this->common->zero_stock()) {
							$avlqty = $this->ewb->getAvailableSerialByPID($product_id[$key],$wid);							
                ...
                switch ($pmethod_id) {
                    case 1 : $r_amt1 = $pamnt;
                        break;
                    case 'Card Swipe' : $r_amt2 = $pamnt;
                        break;
                    case 2 : $r_amt3 = $pamnt;
                        break;
					case 3 : $upi = $pamnt;
                        break;
					case 4 : $rupay_debit_card = $pamnt;
                        break;
					case 5 : $debit_card_others = $pamnt;
                        break;
					case 6 : $credit_card = $pamnt;
                        break;
					case 7 : $emi = $pamnt;
                        break;
                }	
				
				$data = array('type' => 21, 'rid' => $warehouse[0]->cid, 'col1' => $pamnt, 'd_date' => $bill_date, 'invoice_id' => $invocieno);
				$this->db->insert('geopos_metadata', $data);
				$insertedmetaid = $this->db->insert_id();
				 
                if ($pamnt > 0) $this->billing->paynow($invocieno, $pamnt, $tnote, $pmethod, $this->aauth->get_user()->loc, $bill_date, $account,$trans_type=1,$pmethod_id,$insertedmetaid);
                $this->registerlog->update($this->aauth->get_user()->id, $r_amt1, $r_amt2, $r_amt3, 0, $c_amt,$upi,$rupay_debit_card,$debit_card_others,$credit_card,$emi);
                if ($promo_flag) {					
            ...
            if ($this->db->insert('geopos_invoices', $data)) {				
                ...
                foreach ($pid as $key => $value) {
					...
					//if ($product_id[$key] > 0 and $this->common->zero_stock()) {
						$avlqty = $this->invocies->getAvailableQtyByPID($product_id[$key],$wid);
						if((numberClean($avlqty) - $amt) < 0 and $st_c == 0){
							echo json_encode(array('status' => 'Error', 'message' => 'Product - <strong>' . $product_name1[$key] . "</strong> - Low quantity. Available stock is  " . $avlqty));
							$transok = false;
							$st_c = 1;
...
//draft
       ...
       /*  $data = array('type' => 21, 'rid' => $warehouse[0]->cid, 'col1' => $t_profit, 'd_date' => $bill_date);
    ...
    public function ajax_list()
    {
		...
		if($_SESSION['s_role']=="r_4"){
			$list = $this->invocies->get_datatables($this->limited='',$type=2);
		}else{
			$list = $this->invocies->get_datatables($this->limited='',$type=2, $warehouse->id );
		}
		$data = array();
        ...
        foreach ($list as $invoices) {
			...
			$items = $this->invocies->invoice_products($invoices->id);
			//if ($data['invoice']['id']) $data['activity'] = $this->invocies->invoice_transactions($tid);
			$products_html = '<tr><td>';
			foreach($items as $key=>$row){
				$products_html .= '<tr><td>'.$row['product'].'</td></tr>';
			}
			$products_html .= '</td></tr>';
            ...
            $row = array();
            $row[] = $no;
            $row[] = '<a href="' . base_url("pos_invoices/view?id=$invoices->id") . '">&nbsp;' . $prefix.$invoices->tid . '</a>';
            $row[] = $invoices->name;
            $row[] = dateformat($invoices->invoicedate);
			$row[] = $products_html;			
            $row[] = amountExchange($invoices->total, 0, $this->aauth->get_user()->loc);
            $row[] = '<span class="st-' . $invoices->status . '">' . $this->lang->line(ucwords($invoices->status)) . '</span>';
            //$row[] = '<a href="' . base_url("pos_invoices/view?id=$invoices->id") . '" class="btn btn-success btn-sm" title="View"><i class="fa fa-eye"></i> View</a>&nbsp;<a href="' . base_url("pos_invoices/thermal_pdf?id=$invoices->id") . '&d=1" class="btn btn-info btn-sm"  title="Download"><span class="fa fa-download"></span> Download</a>&nbsp;<a href="#" data-object-id="' . $invoices->id . '" class="btn btn-danger btn-sm delete-object"><span class="fa fa-trash"></span></a>';
            $row[] = '<a href="' . base_url("pos_invoices/view?id=$invoices->id") . '" class="btn btn-success btn-sm" title="View"><i class="fa fa-eye"></i> View</a>&nbsp;<a href="' . base_url("pos_invoices/thermal_pdf?id=$invoices->id") . '&d=1" class="btn btn-info btn-sm"  title="Download"><span class="fa fa-download"></span> Download</a>';
    ...
    public function view()
    {
		...
		$billFromId = $data['invoice']['eid'];
		$data['invoice']['from_address'] = $this->invocies->getBillFromAddress($billFromId)[0];
		
		$customerId = $data['invoice']['csd2'];
		$data['invoice']['to_address'] = $this->invocies->getBillToAddress($customerId);
        $data['general'] = array('title' => $this->lang->line('Invoice'), 'person' => 'Bill To', 'prefix' => $pref, 't_type' => 0);
	...
	/*public function printinvoice28721()
    {
        $tid = $this->input->get('id');
        $data['id'] = $tid;
		...
		$billFromId = $data['invoice']['eid'];
        //echo $billFromId; die;
        $address_from = $this->invocies->getBillFromAddress($billFromId);
		$data['invoice']['from_address'] = $address_from[0];
        $data['invoice']['from_address_franchise'] = $address_from[1];
        ...
        if ($data['invoice']['taxstatus'] == 'cgst' || $data['invoice']['taxstatus'] == 'igst') {
            ...
            $html = $this->load->view('print_files/invoice-a4-gst_v1' , $data, true);
        } else {
			//echo "else";die;
   ...
   public function printinvoice()
    ...
    public function printinvoice21062021()
    {
        $tid = $this->input->get('id');
        $data['id'] = $tid;
        $data['round_off'] = $this->custom->api_config(4);
        ...
        if ($ptype == 4) {
            ...
            if ($this->db->update('geopos_invoices', $data)) {
                ...
                switch ($pmethod_id) {
                    case 1 : $r_amt1 = $diff;
                        break;
                    case 'Card Swipe' : $r_amt2 = $diff;
                        break;
                    case 2 : $r_amt3 = $diff;
                        break;
					case 3 : $upi = $diff;
                        break;
					case 4 : $rupay_debit_card = $diff;
                        break;
					case 5 : $debit_card_others = $diff;
                        break;
					case 6 : $credit_card = $diff;
                        break;
					case 7 : $emi = $diff;
                        break;
                }
                ...
                $this->billing->paynow($invocieno, $diff, $tnote, $pmethod, $this->aauth->get_user()->loc, 0, $account,$trans_type=1,$pmethod_id);
                $this->registerlog->update($this->aauth->get_user()->id, $r_amt1, $r_amt2, $r_amt3, 0, $c_amt,$upi,$rupay_debit_card,$debit_card_others,$credit_card,$emi);
                if ($promo_flag) {
                    $cqty = $result_c['available'] - 1;
                    if ($cqty > 0) {
...
//echo $qrCode->writeString();
	...
	public function createNewCustomer(){
		$save = $this->invocies->AddNewCustomer(); //exit;
		if($save == true){
			redirect(base_url().'pos_invoices/create');
		}else{
	...
	public function select_account(){
		...
		$franchise = $this->customers->getFranchchiseBySalesPerson();
		$acc_list = $this->invocies->accountslist();
		
		if($method==1){
			 $str = '<select name="p_account" id="p_account" class="form-control">';
                                foreach ($acc_list as $row) {
									if($row['id']==$franchise->cash_account_id){ $selected='selected'; }else{ $selected=''; }  
                                    $str .= '<option value="' . $row['id'] . '" '.$selected.'>' . $row['holder'] . ' / ' . $row['acn'] . '</option>';
                                }                                
                            $str .= '</select>';
			echo $str;
		}else{
			$str = '<select name="p_account" id="p_account" class="form-control">';
                                foreach ($acc_list as $row) {
									if($row['id']==$franchise->other_account_id){ $selected='selected'; }else{ $selected=''; }  
                                    $str .= '<option value="' . $row['id'] . '" '.$selected.'>' . $row['holder'] . ' / ' . $row['acn'] . '</option>';
                                }                                
                            $str .= '</select>';
			echo $str;
     ...
     public function getSerialPos()
    {
        $wid = $this->input->post('wid');
        $pid = $this->input->post('pid');
        $row_num = $this->input->post('row_num');
       
        $this->db->select("b.*,a.id as warehouse_serial_id");
        $this->db->from("tbl_warehouse_serials as a");
        $this->db->join("geopos_product_serials as b","a.serial_id=b.id",'left');
        $this->db->where('a.twid',$wid);
        ...
        $this->db->where('a.status!=',8);
        $this->db->where('a.is_present',1);
        $this->db->where('a.hold_status',0);
        $this->db->limit(1);
        $query = $this->db->get();
        $data = $query->row_array();
        
        if($data['warehouse_serial_id'])
        {
            $this->db->set('hold_status',1);
            $this->db->where('id',$data['warehouse_serial_id']);
            $this->db->update('tbl_warehouse_serials');

        }
        
         echo $data['serial'].','.$row_num;
