...
*/
     public function edit()
    {
        ...
        $this->db->where('geopos_products.status!=', 5);
        ...
        $data['conditions'] = $this->conditions->conditions_list(); 
        $data['colour_list'] = $this->colours->colours_list();
...
*/
    ...
    public function editproduct()
    {
        ...
        $product_var = $this->input->post("product_var");
        ...
        if ($pid) {
            $this->products->edit($pid, $catid, $warehouse, $product_name, $product_code, $product_price, $factoryprice, $taxrate, $disrate, $product_qty, $product_qty_alert, $product_desc, $image, $unit, $barcode, $code_type, $sub_cat, $brand, $serial,$warehouse_product_name,$warehouse_product_code,$product_specification,$product_model,$hsn_code,$product_id,$product_unit,$product_color,$p_qty,$product_var,$v_type, $v_stock, $v_alert,$w_type, $w_stock, $w_alert);
        }
    }
    ...
    public function warehouseproduct_list()
    {
        ...
        foreach ($list as $prd) {
			...
			if($prd->condition=='Ok' || $prd->condition=='Good'){
				...
				$zo_bulk_price = 0;
				$zo_retail_price = amountExchange($prd->sale_price, 0, $this->aauth->get_user()->loc);
			}
			
			$category_array = explode('&rArr',$prd->c_title);
			$product_name = str_replace("--","-",$prd->product_name);
			$product_array = explode('-',$product_name);		
			
            $row[] = $prd->warehouse_code;
			$row[] = $prd->warehouse_product_code;
			$row[] = $prd->brand;
			$row[] = $category_array[0];
    ...
    public function barcode()
    {
        ...
        if ($pid) {
            ...
            if(strlen($resultz['barcode'])<=12)
            {
                $ctype = $resultz['code_type'];
            }
            else
            {
                $ctype = 'C39';
   ...
   public function view_over()
    {
       ...
       $cat['product_category_array'] = array_reverse(explode("-",substr($this->categories_model->getParentcategory($checkSub[0]->pcat),1)));
        $cat['product_category_array_title'] = $this->categories_model->getCategoryNames($cat['product_category_array']);
       ...
       $check_pcat = $this->db->query("SELECT * FROM `geopos_products` WHERE sub='".$pid."' || pid='".$pid."' GROUP by pcat");
        ...
        $data['producat_cat'] = $cat['product_category_array_title'][0]['title'];
...
//if(!$cat_ware) exit();
    ...
    public function addbundle(){
		...
		$data['taxlist'] = $this->products->tax();
        $data['cat'] = $this->categories_model->category_list();
        $data['units'] = $this->products->units();
		$data['autogencode'] = $this->products->lastbundleid();
        $data['warehouse'] = $this->categories_model->warehouse_list2();
        //$data['custom_fields'] = $this->custom->add_fields(4);
        $data['brand'] = $this->brand->brand_list();
        //$data['products'] = $this->products->getAllProducts();  
		$data['products'] = $this->categories_model->get_warehouseProductById(1);		
        $data['variables'] = $this->units->variables_list();
    ...
    public function productbundleedit()
    {
		...
		$data['mainproducts'] = $this->categories_model->get_warehouseProductById(1);
        $this->load->view('fixed/header', $head);
    ...
    public function add_component_variation()
    {
        $component_id = $this->input->get('id');
        ...
        $head['usernm'] = $this->aauth->get_user()->username;
        $this->load->view('fixed/header', $head);
        $this->load->view('products/item-component-variation', $data);
        $this->load->view('fixed/footer');
    }
    ...
    public function component_edit()
    {
        ...
        $head['usernm'] = $this->aauth->get_user()->username;
        $this->load->model('units_model', 'units');
        //$data['product'] = $this->products->products_list_without_varient(1, $terms);
        $data['product'] = $this->products->products_list(1, $terms);
 ...
 public function update_component()
    ...
    public function new_custom()
    {
        $head['title'] = "New Custom Label";
        $head['usernm'] = $this->aauth->get_user()->username;
        
        $data['product_list'] = $this->products->getCustomList();


        $this->load->view('fixed/header', $head);
        $this->load->view('products/new-custom',$data);
        $this->load->view('fixed/footer');
    }
    ...
    public function adddata()
    {
        ...
        $data['product'] = $this->products->getMainProduct();
        ...
        $this->load->view('products/add-custom-data', $data);
        $this->load->view('fixed/footer');
    }
	public function getproductinfo(){
		$product_id = $this->input->post('id');
		
		$result = $this->products->getvarient($product_id);
		...
		if(is_array($result)){
			...
			foreach($result as $row){
				if($row->unit_name!='')
				{
				$html .= "<option value='".$product_id.'-'.$row->varient_id."'>".$row->unit_name."</option>";  
				}           
			}
			echo $html;
		}else{
   ...
   public function getvarientcolor(){
        ...
        $id_array = explode('-',$id);
        $product_id = $id_array[0];
        $varient_id = $id_array[1];
        
        $result = $this->products->getvarientcolor($product_id,$varient_id);
    ...
    public function add_custom_label()
    {
        ...
        $varient = $this->input->post('varient_id');
        $varient_get = explode('-',$varient);
        $varient_id = $varient_get[1]; 
        ...
        $label_size = $this->input->post('size');
    ...
    { 
        ...
        $data['product_detail'] = $this->products->getCustomList($id);
        $data['label_type'] = $type;
        ini_set('memory_limit', '64M');

            $html = $this->load->view('products/new_custom_label', $data, true);
	...
	public function product_cost(){
		...
		$cost = $this->products->getcostStructureByProductID($pid=3);
     ...
     public function add_new_component_variation()
     {

        $v_type = $this->input->post('v_type');
        $pid = $this->input->post('component_id');
        $this->db->select('*');
        $this->db->where("id",$pid);
        $get_component = $this->db->get("tbl_component");
       
        $get_compo_pro = $get_component->row_array();
        $compo_name =  $get_compo_pro['component_name'];
        ...
        if ($v_type) {
          ...
          foreach ($v_type as $key => $value) {
           ...
           if ($v_type[$key]) {
                       ...
                       $this->db->select('product_name,warehouse_product_code');
                        $this->db->from('geopos_products');
                        $this->db->where('pid', $v_type[$key]);
                        $query = $this->db->get();
                        $r_n = $query->row_array();
                        $data['component_name'] = $compo_name . '-' . $r_n['product_name'];
                        $getComponentName = $compo_name . '-' . $r_n['product_name'];
                        $data['qty'] = numberClean($v_stock[$key]);
                        $data['alert'] = numberClean($v_alert[$key]);
                         
                        
                        $data['warehouse_product_code'] = $r_n['warehouse_product_code'];
                        $wpc = $zupc_code[$key];
                        
                        $data['product_id'] = $v_type[$key];
                        $data['product_code'] = $get_compo_pro['product_code'];
                        $data['merge'] = 1;
                        $data['sub'] = $pid;
                        $data['vb'] = $v_type[$key];
                       $this->db->select('*');
                       $this->db->from("tbl_component");
                       $this->db->where("product_id",$v_type[$key]);
                       $this->db->where("sub",$pid);

                       $main_pro = $this->db->get();
                       if($main_pro->num_rows()==0)
                       { 
                    ...
                    $sub_pid = $getsub1['pid'];
                        ...
                        $this->db->select('a.product_name,a.pid as pro_id,b.name,a.warehouse_product_code');
                        $this->db->from('geopos_products as a');
                        ...
                        foreach($query->result() as $key=>$row)
                        {
                            ...
                            $this->db->where("sub",$pid);
                            $get_Exist_varient = $this->db->get('tbl_component');
                            ...
                            (
                                ...
                                'merge' => 1,
                                'warehouse_product_code' => $row->warehouse_product_code,
                                'product_id' => $row->pro_id,
                                'product_code' => $get_compo_pro['product_code'],
                                'sub'   => $pid          
                            );
                             
                            if($get_Exist_varient->num_rows()==0)
                        { 
                        ...
                        redirect('products/manageproductcomponent');
                    }
                }
			}
		}
		...
		public function remove_duplicate_product_name(){
			$this->products->remove_duplicate_product_name();
		}		
		
		public function update_marginal_tax(){
			$product_details = $this->products->update_marginal_tax();
			/* echo "<pre>";
		...
		public function update_purchase_pid(){			
			$records = $this->products->update_purchase_pid();
			/* echo "<pre>";
			print_r($records);
			echo "</pre>"; */
		}
