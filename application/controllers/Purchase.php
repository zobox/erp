...
class Purchase extends CI_Controller
{
     ...
     public function sparepart_edit()
    {
        ...
        $data['taxlist'] = $this->common->taxlist_edit($data['invoice']['taxstatus']);
        $this->load->view('fixed/header', $head);
        $this->load->view('purchase/sparepart-edit', $data);
        $this->load->view('fixed/footer');

    }
     public function marginal_edit()
    {
        ...
        $this->load->view('fixed/header', $head);
        $this->load->view('purchase/marginal-edit', $data);
        $this->load->view('fixed/footer');
    ...
    public function getListPo($supplier_id=''){
     ...
     public function getListPoItem($invoice_id=''){
		...
		if($data_type==1){
			if($result != false){
				foreach($result as $row){
					$html .= "<option value='".$row->id.'-'.$row->pid."'>".$row->product."</option>";
				}
				echo $html;
			}
        }else{			
	...
	public function getvarientcolor(){
		...
		if(is_array($result)){				
				$html = "<option value=''>--Select--</option>";
			foreach($result as $row){
				$html .= "<option value='".$row->colour_id.'-'.$row->color_pid."'>".$row->colour_name."</option>";             
			}
			echo $html;
		}else{
			return false;
		...
		if($query->num_rows()==0)
		{
		   ...
		   if($data_type==1){
			   ...
			   else if($jobwork_required==2)
			   {
				   ...
				   $this->purchase->goods_product_send_praxo();
			   }
		   }else{
	...
	public function getcomponents(){
		$id1 = $this->input->get('product_id');
		$id_array1 = explode('-',$id1);
		$product_id = $id_array1[1];
		
		$id2 = $this->input->get('varient_id');
		$id_array2 = explode('-',$id2);
		$varient_id = $id_array2[3]; 
		
		$id3 = $this->input->get('colour_id');
		$id_array3 = explode('-',$id3);
		$colour_id = $id_array3[1]; 
		
		if($colour_id){
			$result = $this->purchase->getComponentByPid($product_id,$varient_id,$colour_id);
		}elseif($varient_id){
			$result = $this->purchase->getComponentByPid($product_id,$varient_id);
		}else{
			$result = $this->purchase->getComponentByPid($product_id);
		}		
		//echo $result;
	...
	public function scanapi(){
		...
		if($purchase_id){
			$res = $this->purchase->scanapi($purchase_id);	
		}else{
			echo "Purchase_id Required !";
		}
	}
	
	public function remove_duplicate_serial(){		
		$res = $this->purchase->remove_duplicate_serial();		
	}
	
	
	public function checkSerialAvailability(){
		$serial_no = $this->input->post('serial_no');
		$query = $this->db->query("select * from geopos_product_serials where serial='".$serial_no."'");
