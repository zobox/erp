...
class Projects extends CI_Controller
{
    ...
    public function zobox_service_center()
    {
        $this->db->select("*");
        $query = $this->db->get("tbl_service_center")->result_array();
        $data['service_list'] = $query;
        ...
        $this->load->view('fixed/header', $head);
        $this->load->view('projects/zobox-service-center', $data);
        $this->load->view('fixed/footer');
    ...
    public function project_load_list()
    {
        ...
        foreach ($list as $project) {
			...
			$row[] = '<a href="' . base_url() . 'projects/explore?id=' . $project->id . '" class="btn btn-primary btn-sm rounded" data-id="' . $project->id . '" data-stat="0"> <i class="fa fa-eye"></i>  ' . $this->lang->line('View') . ' </a> ';
			}
            $data[] = $row;
        }
    ...
    public function todo_load_list()
    {
        ...
        foreach ($list as $task) {
			$e_data = $this->employee->employee_details($task->eid);
            $no++;
	...
	public function new_notice()
    {
        ...
        if ($this->input->post()) {
            ...
            $head['usernm'] = $this->aauth->get_user()->username;
            $this->load->view('fixed/header', $head);
            $this->load->view('projects/newnotice', $data);
            $this->load->view('fixed/footer');
        }
    }
