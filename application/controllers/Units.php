...
class Units extends CI_Controller
{
	...
	//conditions
    public function conditions11022021()
    {

        $head['title'] = "Variations conditions";
        $data['units'] = $this->units->conditions_list();
        $head['usernm'] = $this->aauth->get_user()->username;
        $this->load->view('fixed/header', $head);
        $this->load->view('units/conditions', $data);
        $this->load->view('fixed/footer');
    }

	public function create_cn11022021()
    {
        if ($this->input->post()) {
            $name = $this->input->post('name', true);
            $var_id = $this->input->post('pvars');
            ...
            $this->units->create_cn($name, $var_id);
        } else {


            $head['title'] = "Add variation condition";
            $head['usernm'] = $this->aauth->get_user()->username;
            $data['variables'] = $this->units->variables_list();
            $this->load->view('fixed/header', $head);
            $this->load->view('units/create_cn', $data);
            $this->load->view('fixed/footer');
        }
    }

    public function edit_cn11022021()
    {
        if ($this->input->post()) {
            $id = $this->input->post('id');
            $name = $this->input->post('name', true);
            ...
            $this->units->edit_cn($id, $name, $var_id);
        } else {


            $head['title'] = "Edit variation condition";
            $head['usernm'] = $this->aauth->get_user()->username;
            $data = $this->units->view($this->input->get('id'));
            $this->load->view('fixed/header', $head);
            $this->load->view('units/edit_cn', $data);
            $this->load->view('fixed/footer');
        }
    }


    public function delete_cn_i11022021()
    {
        $id = $this->input->post('deleteid');
        if ($id) {
