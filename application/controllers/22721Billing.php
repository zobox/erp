...
class Billing extends CI_Controller
{
    ...
    public function view()
    {
        ...
        if (hash_equals($token, $validtoken)) {
            ...
            $invoice = $this->invocies->invoice_details($tid, '', false);
			...
			if($invoice['type']==2){
				...
				$this->load->model('pos_invoices_model', 'pinvocies');
				$data['invoice'] = $this->pinvocies->invoice_details($tid, $this->limited); 
				
				$billFromId = $data['invoice']['eid'];
				$data['invoice']['from_address'] = $this->pinvocies->getBillFromAddress($billFromId)[0];
				
				$customerId = $data['invoice']['csd2'];
				$data['invoice']['to_address'] = $this->pinvocies->getBillToAddress($customerId);
...
// Set some example data for the payment.
    ...
    public function sparepart_printorder()
    {
        if (!$this->input->get()) {
            exit();
        }
