...
class Invoices_model extends CI_Model
{
	...
	private function _get_datatables_query()
    {
		 ...
		 users_lrp.phone,
		 users_lrp.pincode,
		 users_lrp.gst_number');
        $this->db->from($this->table);
        $this->db->where('users_lrp.users_id', $this->session->userdata('user_details')[0]->users_id);
		//$this->db->where('geopos_invoices.i_class');
        $this->db->join('geopos_warehouse', 'geopos_invoices.twid = geopos_warehouse.id', 'left');
        $this->db->join('users_lrp', 'geopos_warehouse.franchise_id = users_lrp.users_id', 'left');
    ...
    function get_datatables($opt = '',$type='')
    {
        ...
        //$this->db->where('geopos_invoices.csd', $this->session->userdata('user_details')[0]->users_id);
