...
<div class="invoice-box">
    ...
    <table class="party" style="margin-top:30px;">
        <thead>
        <tr class="heading">
            <td> <?php if($invoice['type'] == 2){echo "Bill From";}else{echo $this->lang->line('Our Info');} ?>:</td>
            <td><?php if($invoice['type']==6){ echo "Bill To"; }else{ $general['person']; } ?>:</td>
        </tr>
        </thead>
        <tbody>
        ...
        <tr>
            ...
            <td>
                ...
                if ($invoice['country']) echo '<br>' . $invoice['country'];
                if ($invoice['postbox']) echo ' - ' . $invoice['postbox'];*/
                if ($invoice['phone']) echo $this->lang->line('Phone') . ': ' . $invoice['phone'];
                if ($invoice['email']) echo '<br> ' . $this->lang->line('Email') . ': ' . $invoice['email'];
                if ($invoice['address']) echo '<br>' . $this->lang->line('Tax') . ' ID: ' . $invoice['address'];
                /*if ($invoice['taxid']) echo '<br>' . $this->lang->line('Tax') . ' ID: ' . $invoice['taxid'];*/
                /* if (is_array($c_custom_fields)) {
                    echo '<br>';
                    foreach ($c_custom_fields as $row) {
    ...
    <table class="plist" cellpadding="0" cellspacing="0">
        <tr class="heading">
            ...
            <td><?php echo $this->lang->line('HSN') ?></td>
