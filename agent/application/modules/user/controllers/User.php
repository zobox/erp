...
class User extends CI_Controller
{
	...
	function upload2()
    {
        foreach ($_FILES as $name => $fileInfo) {
            ...
            $exp = explode('.', $filename);
            $ext = end($exp);
            $newname = "agent_upl_" . time() . "." . $ext;
            $config['upload_path'] = '../app-assets/images/agencykyc/';
            $config['upload_url'] = '../app-assets/images/agencykyc/';
            $config['allowed_types'] = "gif|jpg|jpeg|png";
            $config['max_size'] = '2000000';
            $config['file_name'] = $newname;
            $this->load->library('upload', $config);
            move_uploaded_file($tmpname, "../app-assets/images/agencykyc/" . $newname);
            return $newname;
        }
    }
        ...
        if ($id != '') {
            ...
            if ($this->input->post('password') != '') {
                if ($this->input->post('currentpassword') != '') {
                    $old_row = getDataByid('users_agency', $this->input->post('users_id'), 'users_id');
                    if (password_verify($this->input->post('currentpassword'), $old_row->password)) {
                        if ($this->input->post('password') == $this->input->post('confirmPassword')) {
                            $password = password_hash($this->input->post('password'), PASSWORD_DEFAULT);
                            $data['password'] = $password;
							$data['hass_p'] =$this->input->post('confirmPassword');
                        } else {
		   ...
		   $this->User_model->updateRow('users_agency', 'users_id', $id, $data);
