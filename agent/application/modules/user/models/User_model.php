...
class User_model extends CI_Model
{
    ...
    function auth_user()
    {
        ...
        $result = $this->db->get('users_agency')->result();
        if (!empty($result)) {
    ...
    function get_users($userID = '')
    {
		...
		if (isset($userID) && $userID != '') {
			...
			$userId = $this->session->userdata('user_details')[0]->agency_id;
		}
		$userId = $this->session->userdata('user_details')[0]->agency_id;
		$query = $this->db->query("select * from `users_agency` as `b` INNER JOIN `agency_registration` as `a` on `b`.`agency_id` = `a`.`id` AND `a`.`id` = '$userId'");
