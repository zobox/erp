...
class Invoices extends CI_Controller
{
    ...
    public function ajax_list()
    {
        ...
        $list = $this->invocies->get_datatables($type=2);
		/* echo "<pre>";
		print_r($list);
		echo "</pre>"; */
        ...
        foreach ($list as $invoices) {
			if($invoices->type==2){
				if($invoices->pmethod == 'Cash'){
					$invoice_number = 'PCS#'.$invoices->tid;
				}else{
					$invoice_number = 'POS#'.$invoices->tid;
				}				
			}else if($invoices->type==3){
				$invoice_number = 'STF#'.$invoices->tid;
			}else{
				$invoice_number = $invoices->tid;
			}
